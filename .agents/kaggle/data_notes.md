...
## Stage 1 Pipeline-Specific Notes
- Source of truth season target is 2026 (`SampleSubmissionStage2.csv`).
- Pipeline currently uses:
  - Detailed regular season data to build team-season features.
  - Compact NCAA tournament results to build historical training labels.
  - Team spellings to map external CBBD names into Kaggle TeamID.
- Sex split is deterministic:
  - Men TeamIDs are in 1000-1999.
  - Women TeamIDs are in 3000-3999.
- Submission contract:
  - `ID = season_lowTeamID_highTeamID`
  - `Pred = probability lower TeamID beats higher TeamID`
- Data quality expectations implemented in pipeline:
  - Required schema checks for key files.
  - Freshness check that Stage2 sample includes target season.
  - Feature-quality checks before model training/scoring.

  - the next step in pipeline specific implementation should be building out historical data and implementing cbbd api
